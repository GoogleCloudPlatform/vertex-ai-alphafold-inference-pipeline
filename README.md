...
This repository compiles prescriptive guidance and code samples demonstrating how to opertionalize AlphaFold batch inference using Vertex AI Pipelines.
...
The core of the solution is a set of parameterized KFP components that encapsulate key tasks in the AlphaFold inference workflow. Refer to [AlphaFold components summary]() for more detail on the curently available components. 

The AlphaFold KFP components can be composed to implement optimized inference workflows. Currently, the repo contains two example pipelines:
