...
PROJECT_ID = os.environ.get("PROJECT_ID") #'mmm2-355008'  # Change to your project ID
ZONE = os.environ.get("ZONE") #'us-central1-b'   # Change to your zone (example: us-central1-c)
BUCKET_NAME = os.environ.get("BUCKET_NAME") #'alphaf-bucket'  # Change to your bucket name
FILESTORE_ID = os.environ.get("FILESTORE_ID") #'alphaf-nfs' # Change to your Filestore ID
REGION = '-'.join(ZONE.split(sep='-')[:-1])
FILESTORE_SHARE = '/datasets'
AR_REPO_NAME = os.environ.get("AR_REPO_NAME")
FILESTORE_MOUNT_PATH = os.environ.get("FILESTORE_MOUNT_PATH") #'/mnt/nfs/alphafold'
MODEL_PARAMS = f'gs://{BUCKET_NAME}'
IMAGE_URI = f'{REGION}-docker.pkg.dev/{PROJECT_ID}/{AR_REPO_NAME}/alphafold-components' # TODO create AR repo alphafold-components
...
def foldtest():
    ...
    if user_info is not None:
        ...
        print(f'USERINFO before labels{user_info}')
